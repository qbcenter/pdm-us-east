...
class Engine(SimulatedDevice):
    def initialize(self, thing_name, thing_shadow):
        # device_id = thing_shadow['deviceId']
        # tags = device_info['tags']
        properties_desired = thing_shadow['state']['desired']
        properties_reported = thing_shadow['state']['reported'] if 'reported' in thing_shadow['state'] else {}
        ...
        h1_initial = float(properties_desired['h1'])
        h2_initial = float(properties_desired['h2'])
        ...
        self.digital_twin = RotationalMachine(thing_name, h1_gen, h2_gen)
        return True
